...
fileOutput = os.path.join('C:\Users\Ram Asokan\Desktop\Highgard\Scripts\google_analytics','ga_video_plays.csv')
  ...
  try:
    ...
    if not first_profile_id:
      print('Could not find a valid profile for this user.')
		...
		results = get_video_plays(service, first_profile_id)
		get_results(results)
		pg_conn()
...
def get_video_plays(service, profile_id):
  ...
  return service.data().ga().get(
      ...
      filters='ga:eventAction=@Play;ga:eventAction!~list;ga:country==Indonesia,ga:country==Philippines',
      start_index='1',
...
def pg_conn():
  ...
  try:
	...
	with open(fileOutput, 'r') as f:
		cursor.copy_from(f, 'ga_video_plays', sep='^')
	conn.commit()
